...
def test_read_uv_lock_direct_names_dependency_groups(tmp_path: Path) -> None:
    ...
    # Test case for a uv.lock file where a group is not a dictionary
    (tmp_path / "not_a_dict_group.lock").write_text(
        """
        version = 1
        [dependency-groups]
        dev = "just_a_string"
    ...
    assert read_uv_lock(tmp_path / "not_a_dict_group.lock") == {}

    # Test case for a uv.lock file where the "dependencies" key is not a list
    (tmp_path / "not_a_list_group_dependencies.lock").write_text(
        """
        version = 1
        [dependency-groups.dev]
        dependencies = "just_a_string"
        """
    )
    assert read_uv_lock(tmp_path / "not_a_list_group_dependencies.lock") == {}

    # Test case for a uv.lock file where a dependency is not a string
    (tmp_path / "not_a_string_dependency.lock").write_text(
        """
        version = 1
        [dependency-groups.dev]
        dependencies = [123]
...
def test_read_go_mod_single_require(tmp_path: Path) -> None:
    ...
    # Test case for require block with indirect
    go_mod.write_text(
        """
        module example.com/my/module
        go 1.18
        require (
            example.com/other/module v1.2.3 // indirect
        )
        """
    )
    dependencies = read_go_mod(go_mod)
